...
SCRIPT_VERSION = "31-gse39055-assay-quality-diagnostic-v2"
...
def selection_to_gene_expression(
    ...
    # Keep gene_symbol as an explicit column. Selecting rows with
    # .loc[expression.columns] can drop the index name in some pandas
    # versions, causing reset_index() to create an "index" column.
    selected = (
        selected.set_index("gene_symbol", drop=False)
        .loc[expression.columns]
        .reset_index(drop=True)
    )

    if "gene_symbol" not in selected.columns:
        raise RuntimeError(
            "Internal error: gene_symbol was lost during probe selection."
        )
