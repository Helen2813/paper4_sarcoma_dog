...
SCRIPT_VERSION = "38-variable-only-mofapy2-multigroup-v1"

PROJECT_ROOT = Path(__file__).resolve().parents[1]
RESULTS_DIR = PROJECT_ROOT / "results" / "tables"
FIT_ROOT = RESULTS_DIR / "multigroup_mofa_variable_only_fits"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
FIT_ROOT.mkdir(parents=True, exist_ok=True)

CONFIG_FILE = RESULTS_DIR / "multistudy_factor_variable_only_model_config.json"
INPUT_MANIFEST_FILE = RESULTS_DIR / "multistudy_factor_variable_only_manifest.json"
...
OUTPUT_MODEL_INDEX = RESULTS_DIR / "multigroup_mofa_variable_only_model_index.csv"
OUTPUT_FACTOR_ACTIVITY = RESULTS_DIR / "multigroup_mofa_variable_only_factor_activity.csv"
OUTPUT_MODEL_SUMMARY = RESULTS_DIR / "multigroup_mofa_variable_only_model_summary.csv"
OUTPUT_R2 = RESULTS_DIR / "multigroup_mofa_variable_only_variance_explained.csv"
OUTPUT_RANK_SENSITIVITY = RESULTS_DIR / "multigroup_mofa_variable_only_rank_sensitivity.csv"
OUTPUT_CROSS_SET_SENSITIVITY = RESULTS_DIR / "multigroup_mofa_variable_only_cross_set_sensitivity.csv"
OUTPUT_PCA_BASELINE = RESULTS_DIR / "multigroup_mofa_variable_only_stacked_pca_baseline.csv"
OUTPUT_MOFA_VS_PCA = RESULTS_DIR / "multigroup_mofa_variable_only_vs_pca.csv"
OUTPUT_README = RESULTS_DIR / "multigroup_mofa_variable_only_README.txt"
OUTPUT_MANIFEST = RESULTS_DIR / "multigroup_mofa_variable_only_unsupervised_freeze_manifest.json"
...
def main() -> None:
    print("=" * 80)
    print("Python-native variable-only multi-group MOFA factor analysis")
    print("=" * 80)
    print(f"Script version: {SCRIPT_VERSION}")
    print(f"Project root: {PROJECT_ROOT}")
    print("")
    print("Design:")
    print("  Use the official Python mofapy2 multi-group implementation on variable-only feature sets.")
    print("  Treat each cohort as a group and RNA as one shared view.")
    print("  Infer ubiquitous, partially shared, and group-specific factors without forced frozen-module genes.")
    ...
    manifest = {
        ...
        "method": "MOFA2 multi-group, one RNA view, variable-only feature selection",
        ...
        "guardrails": [
            ...
            "Frozen module membership was not used to select the factor-analysis feature space.",
    ...
    print("")
    print("=" * 80)
    print("Variable-only multi-group MOFA model summary")
    print("=" * 80)
    print(
        model_summary[
            [
    ...
    print("The next script may test natural frozen-module recurrence only after this variable-only manifest is frozen.")
