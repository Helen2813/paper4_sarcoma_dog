...
SCRIPT_VERSION = "49-gse239948-blind-de-novo-rediscovery-v2"
...
def clusterable_gene_mask(expression: pd.DataFrame) -> pd.Series:
    variances = expression.var(axis=0, ddof=1)
    finite = np.isfinite(variances.to_numpy(dtype=float))
    variable = variances.to_numpy(dtype=float) > CLUSTER_MIN_VARIANCE
    return pd.Series(finite & variable, index=expression.columns, dtype=bool)
...
def create_manifest(input_paths: list[Path], output_paths: list[Path], r_cut: float) -> None:
    payload = {
        ...
        "cluster_min_variance": CLUSTER_MIN_VARIANCE,
        "guardrails": [
            ...
            "Zero or near-zero variance genes are removed within each subsample before clustering.",
